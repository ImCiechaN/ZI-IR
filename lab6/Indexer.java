...
public class Indexer
{
    ...
    private void indexDocuments() throws IOException {
        ...
        // ----------------------------------
        StandardAnalyzer std_analyzer = new StandardAnalyzer();
        IndexWriterConfig idw_config = new IndexWriterConfig(std_analyzer);
        Directory index = FSDirectory.open(Paths.get(Constants.index_dir));
        IndexWriter writer = new IndexWriter(index, idw_config);
    ...
    private Document getHTMLDocument(String path, int id)
    {
        ...
        */
        ...
        // ----------------------------------
        StoredField notidx_field = new StoredField(Constants.id, id);
        // ----------------------------------
        ...
        Field idx_field = new Field(Constants.content, content, TextField.TYPE_NOT_STORED);
        // ----------------------------------

        // TODO create a field that is stored and indexed
        // and contains file name
        // ----------------------------------
        Field name_field = new Field(Constants.filename, file.getName(), TextField.TYPE_STORED);
        // ----------------------------------

        // TODO create an INT field (IntPoint) that is indexed
        // and contains file size (bytes, .length())
        // ----------------------------------
        IntPoint size_field = new IntPoint(Constants.filesize_int, (int)file.length());
        // ----------------------------------
        // //TODO IntPoint is not stored but we want to a file size
        // ... so add another field (StoredField) that stores file size
        // ----------------------------------
        StoredField size_field_stored = new StoredField(Constants.filesize, file.length());
        // ----------------------------------

        // TODO add fields to the document object
        // ----------------------------------
        document.add(notidx_field);
        document.add(idx_field);
        document.add(name_field);
        document.add(size_field);
        document.add(size_field_stored);
        // ----------------------------------
