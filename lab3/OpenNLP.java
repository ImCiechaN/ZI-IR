...
public class OpenNLP {
	...
	public static String TOKENIZER_DE = "models/de-token.bin";
	...
	private void languageDetection() throws IOException
	{
		...
		System.out.println(me.predictLanguage(text));
	...
	private void tokenization() throws IOException
	{
		...
		System.out.println(Arrays.toString(me.tokenize(text)));
		System.out.println(Arrays.toString(me.getTokenProbabilities()));
	}
	...
	private void sentenceDetection() throws IOException
	{
		...
		//text = "Hi??";
		System.out.println(Arrays.toString(me.sentDetect(text)));
	}
	...
	private void posTagging() throws IOException {
		...
		sentence = new String[] {"Hi", "Hey", "Hello", "milk", "??????"};
		...
		System.out.println(Arrays.toString(me.tag(sentence)));
	}
	...
	private void lemmatization() throws IOException
	{
		...
		tags = new String[] { "NNP", "WRB", "VBP", "PRP", "VB", "TO", "VB", "PRP", "VB", "JJ", "JJ", "NNS", "IN", "JJ",
				"NN", "VBG" };
		System.out.println(Arrays.toString(lem.lemmatize(text, tags)));
	...
	private void stemming()
	{
		...
		sentence = new String[] { "Hi", "How", "are", "you", "Welcome", "to", "OpenNLP", "We", "provide", "multiple",
				"built-in", "methods", "for", "Natural", "Language", "Processing" };
		String[] stemmed = new String[sentence.length];
		for(int i = 0; i < sentence.length; i ++){
			stem.stem(sentence[i]);
			stemmed[i] = stem.toString();
			stem.reset();
		}
		System.out.println(Arrays.toString(stemmed));
	...
	private void chunking() throws IOException
	{
		...
		ChunkerModel chunk = new ChunkerModel(modelFile);
		ChunkerME me = new ChunkerME(chunk);
		String[] sentence = new String[0];
		sentence = new String[] { "She", "put", "the", "big", "knives", "on", "the", "table" };
		...
		System.out.println(Arrays.toString(me.chunk(sentence, tags)));
	...
	private void nameFinding() throws IOException
	{
		...
		File modellFile = new File(TOKENIZER_DE);
		TokenizerModel modell = new TokenizerModel(modellFile);
		TokenizerME mee = new TokenizerME(modell);
		...
		String[] tokens = mee.tokenize(text);
		System.out.println(Arrays.toString(tokens));
		opennlp.tools.util.Span[] span = me.find(mee.tokenize(text));
